...
class CoreStateHostViewImpl implements MutableCoreStateView {
  ...
  readReg32(reg: Reg32): number {
    return this.#storage.readArrayElement(coreStateFields.gprs, reg);
  }

  writeReg32(reg: Reg32, value: number): void {
    this.#storage.writeArrayElement(coreStateFields.gprs, reg, value);
  }

  readSegmentSelector(reg: SegmentRegister): number {
    return this.#storage.readArrayElement(coreStateFields.segmentSelectors, reg);
  }

  writeSegmentSelector(reg: SegmentRegister, value: number): void {
    this.#storage.writeArrayElement(coreStateFields.segmentSelectors, reg, value);
  }

  readSegmentBase(reg: SegmentRegister): number {
    return this.#storage.readArrayElement(coreStateFields.segmentBases, reg);
  }

  writeSegmentBase(reg: SegmentRegister, value: number): void {
    this.#storage.writeArrayElement(coreStateFields.segmentBases, reg, value);
  }

  readSegmentLimit(reg: SegmentRegister): number {
    return this.#storage.readArrayElement(coreStateFields.segmentLimits, reg);
  }

  writeSegmentLimit(reg: SegmentRegister, value: number): void {
    this.#storage.writeArrayElement(coreStateFields.segmentLimits, reg, value);
  }

  readSegmentAccess(reg: SegmentRegister): number {
    return this.#storage.readArrayElement(coreStateFields.segmentAccess, reg);
  }

  writeSegmentAccess(reg: SegmentRegister, value: number): void {
    this.#storage.writeArrayElement(coreStateFields.segmentAccess, reg, value);
  }
}
