...
test("memory guard mode rejects missing operand storage metadata", () => {
  throws(
    () => buildIr(aluSemantic("add", 32), { memoryGuards: true }),
    /memory guard requires storage metadata for operand 0/
  );
});
