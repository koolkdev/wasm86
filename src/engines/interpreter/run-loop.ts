...
export function encodeRunLoopBody(
  ...
  body.block();
  emitOpcodeFetch({ body, scratch, bindings }, { eipLocal: locals.eip, byteLocal: locals.byte });
  // Fault paths commit the start; prefix cases rebase the eip local.
  body.localGet(locals.eip).localSet(locals.instructionStart);
  emitPrefixStateReset({ body, scratch, bindings, locals });
  // Prefix cases branch here to rescan with their prefix folded into locals.
  body.loop();
  emitOpcodeDispatch({ body, scratch, bindings, locals, handlers, continueDepth: 1, rmDecode });
  body.endBlock();
  body.endBlock();
