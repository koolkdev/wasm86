...
test("Virtual binding clears and identity-maps the complete live RAM backing", () => {
  ...
  const tableBytes = new Uint8Array(machine.buffer, entries.offset, entries.stride * entries.count);
  ...
  let mismatch: Readonly<{ index: number; actual: number; expected: number }> | undefined;

  for (let index = 0; index < entries.count; index += 1) {
    const actual = view.getUint32(entries.offset + index * entries.stride, true);
    const expected = index < mappedPageCount ? index * 0x1000 + 0x3 : 0;
...
test("Virtual binding reads mapped RAM without mapping later growth", () => {
  ...
  deepStrictEqual(bound.reader.readByte(finalMappedAddress, "instructionFetch"), {
    kind: "value",
    value: 0xa5
  });
  ...
  deepStrictEqual(bound.reader.readByte(previousByteLength, "instructionFetch"), {
    kind: "exception",
    exception: {
      kind: "PF",
