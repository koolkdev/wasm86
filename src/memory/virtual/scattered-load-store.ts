...
export function createScatteredLoadStore(
  ...
  const functions = createScatteredLoadStoreFunctions(
    physical,
    pageTable
  );
  ...
  return {
    load: (
      ...
      const raw = region.call(
        functions.reads.get(width),
        [linearStart]
      )[0];
...
function createScatteredLoadStoreFunctions(
  ...
  return {
    reads: new FunctionFamily<IntegerWidth>({
      ...
      build: (width, fn) =>
        buildScatteredRead(fn, width, physical, pageTable)
    }),
    stores: new FunctionFamily<IntegerWidth>({
      ...
      build: (width, fn) =>
        buildScatteredStore(fn, width, physical, pageTable)
    })
  };
}
...
function buildScatteredRead(
  ...
  fn.return([
    readBytes(fn.region, physical, pageTable, linearStart, byteLength)
  ]);
}
...
function readTranslatedByte(
  ...
  return readPhysicalByte(
    region,
    physical,
    translateAddress(region, linearAddress, pageTable)
  );
}
...
function translateAddress(
  ...
  return translate(
    region,
    linearAddress,
    pageTable.read(region, pageIndex(region, linearAddress))
  );
}
...
function translate(
  ...
  const frame = region.values.binary(
    "and",
    entry,
...
function addByteOffset(
  ...
  return byteOffset === 0
    ...
    : region.values.binary(
        "add",
        address,
        region.values.const(byteOffset)
      );
}
