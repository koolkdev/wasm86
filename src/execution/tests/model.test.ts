...
test("execution model composes owner-provided definitions", () => {
  ...
  ok(model.resources.memoryImports.includes(model.memory.physical.ramImport));
  ok(model.resources.memoryImports.includes(model.memory.machineMemory.memoryImport));
  strictEqual(model.memory.machineMemory.memoryImport.ref, model.memory.machineMemory.resource);
...
test("execution models use distinct symbolic resource identities", () => {
  ...
  notStrictEqual(first.memory.physical.ramResource, second.memory.physical.ramResource);
  notStrictEqual(first.memory.physical.access, second.memory.physical.access);
  notStrictEqual(first.memory.machineMemory.resource, second.memory.machineMemory.resource);
  notStrictEqual(first.memory.access, second.memory.access);
  strictEqual(
    first.resources.memoryImports.some((memory) => memory.ref === second.cpuState.resource),
    false
  );
});
