...
test("every variable has a fresh opaque identity", () => {
  const first = new VariableRef(32);
  const second = new VariableRef(32);

  notStrictEqual(first, second);
  notStrictEqual(first, new VariableRef(64));
});

test("a variable exposes its logical integer width", () => {
  const variable = new VariableRef(64);

  strictEqual(variable.kind, "variable");
  strictEqual(variable.width, 64);
});
