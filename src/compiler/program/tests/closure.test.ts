...
test("closure retains only resources used by live operations", () => {
  ...
  const program = new ProgramBuilder(
    createProgramResources([unusedMemory, usedMemory])
  );
...
test("function resources describe direct operations rather than callee effects", () => {
  ...
  deepStrictEqual(
    closed.functions.find((fn) => fn.ref === callee.ref)?.resources,
    [memory.ref]
  );
  deepStrictEqual(
    closed.functions.find((fn) => fn.ref === caller.ref)?.resources,
    []
  );
});
...
test("closure retains reachable function-family members only", () => {
  ...
  strictEqual(closed.functions.some((fn) => fn.ref === root.ref), true);
  strictEqual(closed.functions.some((fn) => fn.ref === live.ref), true);
  strictEqual(closed.functions.some((fn) => fn.ref === transitive.ref), true);
