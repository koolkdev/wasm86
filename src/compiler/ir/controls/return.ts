...
export type ReturnControl = ControlNodeBase &
  Readonly<{
    kind: "return";
    source: ReturnSource;
  }>;

export const returnControl: ControlDefinition<ReturnControlArgs, ReturnControl> = {
  kind: "return",
  create: ({ source }) => ({
    category: "control",
    kind: "return",
  ...
  describe: (control) => {
    ...
    return {
      ...
      outputs: [],
      nestedBodies: [],
      effects: source.kind === "values" ? noStorageEffects : source.invocation.target.effects
    };
  },
