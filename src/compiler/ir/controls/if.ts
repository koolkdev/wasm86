...
import type { BranchHint, ControlDefinition, ControlNodeBase } from "./definition.js";
...
export type IfControl = ControlNodeBase &
  Readonly<{
    kind: "if";
    condition: ValueId;
    hint?: BranchHint;
...
export const ifControl: ControlDefinition<IfControlArgs, IfControl> = {
  ...
  mapBodies: (control, map) =>
    ifControl.create({
      ...
      ...(control.elseBody === undefined ? {} : { elseBody: map(control.elseBody) })
    }),
