...
import { BodyBuilder, buildIrBlock } from "#ir/body-builder.js";
import { eipChannel } from "#ir/slots.js";
import { validateIrBlock } from "#ir/validate.js";
import { fitsUnsigned, unboundedWidthBounds, ValueTable, type ValueId } from "#ir/values.js";
import { memoryCheck, resolveFlag, stateWrite } from "#ir/tests/storage-op-helpers.js";
...
test("op derives the output and its bounds from the op access", () => {
  ...
  const fault = builder.op({ kind: "memory.check", address, byteLength: 4, access: "read" });
...
test("op rejects output-less ops and effect rejects output-bearing ones", () => {
  ...
  throws(() => builder.effect({ kind: "state.read", slot: eipChannel }), /state\.read op has an output/);
});

test("effect and push append actions without outputs", () => {
  const values = new ValueTable();
  const builder = new BodyBuilder(values);
  const prebuilt = stateWrite(eipChannel, values.const(4));

  builder.effect({ kind: "state.write", slot: eipChannel, value: values.const(0) });
  builder.push(prebuilt);
  deepStrictEqual(builder.build(), {
    actions: [stateWrite(eipChannel, values.const(0)), prebuilt]
  });
});
...
test("if builds hinted then and else bodies against child builders", () => {
  ...
  builder.if(
    condition,
    (then) => then.effect({ kind: "state.write", slot: eipChannel, value: then.values.const(4) }),
    {
      hint: "unlikely",
...
test("switch builds every arm before allocating the shared output", () => {
  ...
  const output = builder.switch(
    selector,
    [{ match: 3, build: (arm) => (armResult = arm.op({ kind: "cpu.resolveFlag", flag: "ZF" })) }],
...
test("buildIrBlock forwards a returned value as the root body result", () => {
  let eip!: ValueId;
  const block = buildIrBlock((b) => (eip = b.op({ kind: "state.read", slot: eipChannel })));
...
test("buildIrBlock leaves the root result unset for void callbacks", () => {
  const block = buildIrBlock((b) => {
    b.effect({ kind: "state.write", slot: eipChannel, value: b.values.const(0) });
  });
