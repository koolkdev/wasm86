...
test("instruction operands agree with their encoded ordering and opcode fields", () => {
  for (const instruction of X86_32_CORE.instructions) {
    ...
    const relativeIndex = operands.findIndex((operand) => operand.kind === "rel");
    const hasOpcodeRegister = operands.some((operand) => operand.kind === "opcode.reg");
    const hasOpcodeLowBits = instruction.opcode.some(
      (part) => typeof part !== "number" && (part.bits ?? 8) < 8
    );
    ...
    strictEqual(hasOpcodeRegister, hasOpcodeLowBits, `${instruction.id} opcode register binding`);
  }
});
